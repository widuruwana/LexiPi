...
static thread_local Spy spy; // Thread-local instance
...
struct FastBag {
    ...
    // [FIX] Correctly initialize from vector
...
static int computeFullScore(const LetterBoard& board, const Board& bonusBoard, const MoveCandidate& move) {
    ...
    for (int i = 0; move.word[i] != '\0'; i++) {
        ...
        if (isNew) {
            ...
            if (hasNeighbor) {
                ...
                int crossMult = 1;
                int cr = r, cc = c;
                while(cr - pdr >= 0 && cc - pdc >= 0 && board[cr - pdr][cc - pdc] != ' ') {
                    cr -= pdr; cc -= pdc;
                }
                while(cr < 15 && cc < 15) {
                    char l;
                    if (cr == r && cc == c) {
                        l = letter;
                        int cv = (islower(l)) ? 0 : Heuristics::getTileValue(l);
                        CellType cb = bonusBoard[cr][cc];
                        if (cb == CellType::DLS) cv *= 2;
                        else if (cb == CellType::TLS) cv *= 3;
                        if (cb == CellType::DWS) crossMult *= 2;
                        else if (cb == CellType::TWS) crossMult *= 3;
                        crossScore += cv;
                    } else {
                        l = board[cr][cc];
                        if (l == ' ') break;
    ...
    if (strlen(move.word) > 1) {
        totalScore += (mainWordScore * mainWordMult);
    }
...
static float calculateRackLeave(const LetterBoard& board, const int* rackCountsInput, const MoveCandidate& move) {
    ...
    for(int i = 0; i < 26; i++) {
        if (rackCounts[i] > 0) {
            char tile = (char)('A' + i);
            leaveVal += Heuristics::getLeaveValue(tile);
        }
    }
...
static int fastPlayout(LetterBoard currentBoard, const Board& bonusBoard,
                       int* myRackCounts, int* oppRackCounts, FastBag bag, bool myTurn, Dawg& dict) {
    ...
    for (int turn = 0; turn < maxTurns; turn++) {
        ...
        auto consumer = [&](MoveCandidate& cand, int* rack) -> bool {
            ...
            if (myTurn) {
                float leave = calculateRackLeave(currentBoard, rack, cand);
                metric += (leave * 0.5f);
            }

            if (metric > bestMetric) {
                bestMove = cand;
                bestMove.score = (short)realScore;
                bestMetric = metric;
                found = true;
            }
            return true;
...
MoveCandidate Vanguard::search(const LetterBoard& board, const Board& bonusBoard,
    ...
    for (unsigned int t = 0; t < nThreads; t++) {
        workers.push_back(async(launch::async,
            [&, t]() {
                // [FIX] INITIALIZE SPY ONCE PER THREAD (OUTSIDE THE LOOP)
                // This performs the expensive "Ghost Search" and Probability Updates
                // just one time. The 'spy' state is then frozen for the simulation.
                spy.updateGroundTruth(board, myRackVec);
                spy.performInference(board, 20, dict);
                ...
                while (true) {
                    ...
                    // [FIX] Generate Rack using the pre-calculated Spy beliefs
                    // This is now fast and memory-safe
                    int oppRackCounts[27] = {0};
                    spy.generateWeightedRack(oppRackCounts);
                    ...
                    for(int i=0; move.word[i]!='\0'; ++i) {
                        if(nextBoard[r][c] == ' ') {
                            ...
                            if(l >= 'a' && l <= 'z') { if(myResidualRack[26]>0) myResidualRack[26]--; }
                                ...
                                int idx = toupper(l) - 'A';
                                if(myResidualRack[idx]>0) myResidualRack[idx]--;
                                else if(myResidualRack[26]>0) myResidualRack[26]--;
                            }
                        }
                        r += dr; c += dc;
                    }
