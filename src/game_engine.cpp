...
void runGame(PlayerController* p1, PlayerController* p2, bool shuffleBag) {
    ...
    // If not shuffling, we assume it's a test mode and want the arranged bag.
    TileBag bag = createStandardTileBag(!shuffleBag);
    if (shuffleBag) {
        shuffleTileBag(bag);
    }
    ...
    printTitle();
    cout << "Welcome to Terminal Crossword Game\n";
